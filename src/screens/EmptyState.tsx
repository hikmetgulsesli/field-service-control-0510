      ...
      <nav className="hidden md:flex flex-col h-full py-lg px-md space-y-sm w-64 bg-surface-container-low border-r border-outline-variant flex-shrink-0 z-40">
        ...
        <div className="flex items-center gap-sm mb-xl px-sm">
          <div className="w-10 h-10 rounded bg-surface-container-highest flex items-center justify-center flex-shrink-0 overflow-hidden">
            <img alt="Organization Logo" className="w-full h-full object-cover" src="https://lh3.googleusercontent.com/aida-public/AB6AXuBGi8DclAnI61tO60SJInJT2M8jGfNjt_ALFkYMpXOeWHXV1WncZkZJiG9cK3t93ufxFMIvpIgZ7ZwrTPw20zowNMx0fPk6W7VTVH5Sw4JZDzmdzXht0dst7IhdcWEN34cEYF_LZ2BE0BAxfZ7-lvcQjDZo76S9205wmN4Jzyov0GEFlzQKfLqRWqncU7hDbnBQbeYBz3N_XrUa08ERx0zjvdY6nG2d3Crz8qBa8SL610azqor_QD169daowOwvTwyVK_mDgvsLdtSf" />
          </div>
        ...
        <div className="flex-1 space-y-xs overflow-y-auto">
          ...
          <a
            ...
            className="flex items-center gap-md px-md py-sm bg-secondary-container text-on-secondary-container font-label-md text-label-md font-bold rounded-lg scale-95 active:scale-100 transition-transform duration-150 cursor-pointer"
            href="#"
          >
          ...
          <a
            onClick={(e) => { e.preventDefault(); actions.navigate("dashboard"); }}
            className="flex items-center gap-md px-md py-sm text-on-surface-variant font-label-md text-label-md hover:bg-surface-container-highest transition-all scale-95 active:scale-100 duration-150 rounded-lg cursor-pointer"
            href="#"
          >
      ...
      {mobileMenuOpen && (
        <div className="fixed inset-0 z-50 md:hidden">
          ...
          <nav className="absolute left-0 top-0 h-full w-64 bg-surface-container-low border-r border-outline-variant flex flex-col py-lg px-md space-y-sm">
            ...
            <button onClick={() => { actions.navigate("create"); setMobileMenuOpen(false); }} className="w-full mb-lg py-md px-md bg-primary-container text-on-primary-container font-label-md text-label-md rounded hover:bg-inverse-primary transition-colors flex items-center justify-center gap-sm shadow-[0px_4px_12px_rgba(0,0,0,0.5)]">
              <span className="material-symbols-outlined text-[18px]">add</span>
              New Record
            </button>
            <div className="flex-1 space-y-xs overflow-y-auto">
              <a onClick={(e) => { e.preventDefault(); actions.navigate("dashboard"); setMobileMenuOpen(false); }} className="flex items-center gap-md px-md py-sm bg-secondary-container text-on-secondary-container font-bold rounded-lg cursor-pointer" href="#">Dashboard</a>
              <a onClick={(e) => { e.preventDefault(); actions.navigate("dashboard"); setMobileMenuOpen(false); }} className="flex items-center gap-md px-md py-sm text-on-surface-variant hover:bg-surface-container-highest rounded-lg cursor-pointer" href="#">Service Records</a>
              <a onClick={(e) => { e.preventDefault(); actions.navigate("insights"); setMobileMenuOpen(false); }} className="flex items-center gap-md px-md py-sm text-on-surface-variant hover:bg-surface-container-highest rounded-lg cursor-pointer" href="#">Insights</a>
              <a onClick={(e) => { e.preventDefault(); actions.navigate("settings"); setMobileMenuOpen(false); }} className="flex items-center gap-md px-md py-sm text-on-surface-variant hover:bg-surface-container-highest rounded-lg cursor-pointer" href="#">Settings</a>
              <a onClick={(e) => { e.preventDefault(); actions.navigate("profile"); setMobileMenuOpen(false); }} className="flex items-center gap-md px-md py-sm text-on-surface-variant hover:bg-surface-container-highest rounded-lg cursor-pointer" href="#">Profile</a>
            </div>
          </nav>
        </div>
      )}
      ...
      <div className="flex-1 flex flex-col min-w-0 h-full relative">
        ...
        <header className="sticky top-0 z-50 flex items-center justify-between px-gutter w-full h-16 bg-surface-container border-b border-outline-variant flex-shrink-0">
          ...
          <div className="flex items-center gap-md">
            <button onClick={() => setMobileMenuOpen(true)} className="md:hidden text-on-surface-variant hover:bg-surface-container-high transition-colors p-sm rounded active:opacity-80 transition-opacity">
          ...
          <div className="flex items-center gap-sm">
            <button onClick={() => actions.openPanel("notifications")} className="text-on-surface-variant hover:bg-surface-container-high transition-colors p-sm rounded active:opacity-80 transition-opacity focus:outline-none focus:ring-2 focus:ring-primary-container" aria-label="Notifications">
              <span className="material-symbols-outlined">notifications</span>
            </button>
            <button onClick={() => actions.navigate("settings")} className="text-on-surface-variant hover:bg-surface-container-high transition-colors p-sm rounded active:opacity-80 transition-opacity focus:outline-none focus:ring-2 focus:ring-primary-container" aria-label="Help">
              <span className="material-symbols-outlined">help_outline</span>
            </button>
            <div onClick={() => actions.navigate("profile")} className="ml-sm w-8 h-8 rounded-full bg-surface-container-highest overflow-hidden border border-outline-variant cursor-pointer active:opacity-80 transition-opacity">
